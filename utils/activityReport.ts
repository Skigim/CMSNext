 ...
 * ====================================
 * Generates formatted activity reports for case tracking and auditing.
 * Supports daily activity summaries, filtering, and export in JSON/CSV formats.
 * 
 * ## Features
 * 
 * - **Daily Summaries**: Aggregate case activity by date
 * - **Case Breakdown**: Show activity per case with counts by type
 * - **Activity Details**: Status changes, priority updates, notes added
 * - **Filtering**: Filter entries by specific date
 * - **Export Formats**: JSON and CSV export capabilities
 * - **Sorting**: Sorts entries by most recent first and cases by activity volume
 * 
 * ## Report Structure
 * 
 * A daily activity report contains:
 * - **Date**: The date the report covers
 * - **Totals**: Aggregate counts (total, status changes, priority changes, notes added)
 * - **Entries**: All activity log entries for the day, sorted by timestamp
 * - **Cases**: Per-case breakdown with activity counts and relevant entries
 * 
 * ## Usage Example
 * 
 * ```typescript
 * const report = generateDailyActivityReport(activityLog, '2025-12-29');
 * 
 * // Get top 3 most active cases
 * const topCases = getTopCasesForReport(report, 3);
 * 
 * // Export as JSON or CSV
 * const json = convertReportToFormat(report, 'json');
 * const csv = convertReportToFormat(report, 'csv');
 * ```
 * 
 * @module activityReport
 * @see {@link CaseActivityEntry} for activity log entry structure
 * @see {@link DailyActivityReport} for report structure
 */
