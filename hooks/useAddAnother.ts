 ...
 * - Component decides whether checkbox state should reset as part of its own reset logic
 * 
...
export interface UseAddAnotherOptions {
  ...
  itemLabel?: string;
  /** Callback to reset form to initial state (may preserve or reset addAnother, per component needs) */
  resetForm: () => void;
 ...
 * - Deciding whether addAnother should reset when the form resets
 * 
 * The hook provides:
...
export function useAddAnother({
  ...
  const handlePostSave = useCallback(
    <T extends HTMLElement>(focusRef?: RefObject<T>) => {
      if (addAnother) {
        // Reset form for another entry while preserving any component-specific checkbox behavior
        resetForm();
        // Show feedback toast
