...
if (isAllowedStartupTool(toolName)) {
  respond(
    "allow",
    "Startup guard still pending. Load repo-memories, then skill-governance, then any more specific applicable skill, and run `node .github/hooks/scripts/mark-startup-complete.mjs <skill-name|none>`."
  );
  process.exit(0);
}
...
respond(
  "deny",
  "CMSNext startup guard: load repo-memories, then skill-governance, then any more specific applicable skill, and run `node .github/hooks/scripts/mark-startup-complete.mjs <skill-name|none>` before using other tools."
);
