...
- Load `skills/using-superpowers/SKILL.md` before any other storage reasoning, then load `repo-memories`, then check for an applicable Superpowers skill before any other storage task action.
...
- Do not rationalize, clarify, inspect storage flows, or implement before the memory check and current skill-selection decision are complete.
