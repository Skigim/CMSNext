...
Load `skills/using-superpowers/SKILL.md` before any other triage reasoning, then load `repo-memories`, then load `skill-governance` to choose applicable skill coverage before any other action.
...
- Do not rationalize, clarify, explore, or route work before the memory check and current `skill-governance` decision are complete.
